...
public class KeywordManagerScreen extends Screen {
    private final Screen parent;
    private TextFieldWidget newKeywordField;
    private TextFieldWidget newColorField;
    private final List<KeywordEntry> keywordEntries = new ArrayList<>();
    private int scrollOffset = 0;
    private static final int ENTRIES_PER_PAGE = 8;


    public KeywordManagerScreen(Screen parent) {
        super(Text.literal("关键词管理"));
        this.parent = parent;
    }
    ...
    protected void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(
            Text.literal("新关键词:"),
            button -> {}
        ).dimensions(centerX - 200, startY, 80, 20).build());
        
        newKeywordField = new TextFieldWidget(this.textRenderer, centerX - 110, startY, 100, 20, Text.literal(""));
        newKeywordField.setMaxLength(50);
        newKeywordField.setPlaceholder(Text.literal("输入关键词..."));
        this.addDrawableChild(newKeywordField);
        
        this.addDrawableChild(ButtonWidget.builder(
            Text.literal("颜色:"),
            button -> {}
        ).dimensions(centerX, startY, 40, 20).build());
        
        newColorField = new TextFieldWidget(this.textRenderer, centerX + 50, startY, 80, 20, Text.literal(""));
        newColorField.setMaxLength(7);
        newColorField.setPlaceholder(Text.literal("#FFFFFF"));
        newColorField.setText("#00FFFF");
        ...
        ).dimensions(centerX + 140, startY, 50, 20).build());
        ...
        this.addDrawableChild(ButtonWidget.builder(
            Text.literal("↓"),
            button -> {
                if (scrollOffset < Math.max(0, ConfigManager.getConfig().blockHighlighter.keywords.size() - ENTRIES_PER_PAGE)) {
                    scrollOffset++;
                    refreshKeywordList();
                }
            }
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.literal("保存"), button -> {
            ...
            if (this.client.player != null) {
                this.client.player.sendMessage(Text.literal("关键词配置已保存").formatted(Formatting.GREEN), false);
            }
    ...
    private void addKeyword() {
        String keyword = newKeywordField.getText().trim().toLowerCase();
        String colorHex = newColorField.getText().trim();
        
        if (keyword.isEmpty()) {
            if (this.client.player != null) {
                this.client.player.sendMessage(Text.literal("关键词不能为空").formatted(Formatting.RED), false);
            }
            return;
        }
        ...
        try {
            ...
            ConfigManager.getConfig().blockHighlighter.keywords.put(keyword, colorHex);
            newKeywordField.setText("");
            refreshKeywordList();
            if (this.client.player != null) {
                this.client.player.sendMessage(Text.literal("已添加关键词: " + keyword).formatted(Formatting.GREEN), false);
            }
    ...
    private void refreshKeywordList() {
        ...
        for (String keyword : keywordList) {
            ...
            KeywordEntry entry = new KeywordEntry(keyword, keywords.get(keyword), this.width / 2 - 180, currentY);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, 
            Text.literal("管理方块高亮的关键词和颜色"), this.width / 2, 35, 0xCCCCCC);
        
        // 绘制关键词列表标题
        context.drawTextWithShadow(this.textRenderer, "关键词", this.width / 2 - 180, 75, 0xFFFFFF);
    ...
    private class KeywordEntry extends ButtonWidget {
        ...
        public KeywordEntry(String keyword, String colorHex, int x, int y) {
            super(x, y, 360, 20, Text.literal(keyword), button -> {}, DEFAULT_NARRATION_SUPPLIER);
        ...
        private void updateKeywordColor() {
            ...
            try {
                ...
                ConfigManager.getConfig().blockHighlighter.keywords.put(keyword, newColor);
                refreshKeywordList();
                if (MinecraftClient.getInstance().player != null) {
                    MinecraftClient.getInstance().player.sendMessage(
                        Text.literal("已更新关键词颜色: " + keyword).formatted(Formatting.GREEN), false);
                }
            } catch (NumberFormatException e) {
                if (MinecraftClient.getInstance().player != null) {
                    MinecraftClient.getInstance().player.sendMessage(
        ...
        private void deleteKeyword() {
            ConfigManager.getConfig().blockHighlighter.keywords.remove(keyword);
            refreshKeywordList();
            if (MinecraftClient.getInstance().player != null) {
                MinecraftClient.getInstance().player.sendMessage(
                    Text.literal("已删除关键词: " + keyword).formatted(Formatting.YELLOW), false);
            }
        }
        ...
        public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            context.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, 
                keyword, this.getX(), this.getY() + 5, 0xFFFFFF);
